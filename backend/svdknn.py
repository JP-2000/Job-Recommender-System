...
rating_df = Dataset.load_from_df(df[['userId','companyId','rating']], reader)
...
def prediction(algo, users_K):
    ...
    for userId in range(1,users_K):
        for companyId in range(1,10000):
    ...
    pred_df = pd.DataFrame(pred_list, columns = ['userId', 'companyId', 'rating'])
    return pred_df
...
pred_SVD = prediction(algo_SVD, 610)
...
recommendations_df= sorted_df[sorted_df['userId']==1]
